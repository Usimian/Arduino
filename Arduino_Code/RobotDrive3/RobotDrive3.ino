...
const int ReadBatteryLevel = 6;   // Read battery level <ReadBatteryLevel>, return <battery voltage (V)>
...
void setup()
{
  ...
  pinMode(PIN_A2, INPUT);       // Battery voltage divider
  ...
  myPIDRight.SetProportionalMode(QuickPID::pMode::pOnMeas);
  myPIDRight.SetMode(QuickPID::Control::timer);
  myPIDRight.SetOutputLimits(-255.0, 255.0);
  myPIDRight.SetSampleTimeUs(100000);

  myPIDLeft.SetTunings(Kp, Ki, Kd);
  myPIDLeft.SetProportionalMode(QuickPID::pMode::pOnMeas);
  myPIDLeft.SetMode(QuickPID::Control::timer);
  ...
  myPIDRightVel.SetProportionalMode(QuickPID::pMode::pOnMeas);
  myPIDRightVel.SetMode(QuickPID::Control::timer);
  myPIDRightVel.SetOutputLimits(0.0, 255.0);
  myPIDRightVel.SetSampleTimeUs(100000);

  myPIDLeftVel.SetTunings(KpV, KiV, KdV);
  myPIDLeftVel.SetProportionalMode(QuickPID::pMode::pOnMeas);
  myPIDLeftVel.SetMode(QuickPID::Control::timer);
...
if (recvWithStartEndMarkers()) // Get command
  {
    ...
    else if (cmd == ReadBatteryLevel) // Return battery voltage (V)
    {
      Serial.print("<");
...
float getBatteryVoltage() // Read voltage divider
{
  uint32_t  batt;
  float     vBatt;

  batt = analogRead(PIN_A2);
  vBatt = (float)batt*(13.5/383);  // Compute voltage

  return (float)vBatt; // Current battery voltage
}
